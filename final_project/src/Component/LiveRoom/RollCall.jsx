...
export const RollCall = (props) => {
	const { userList, videoRef, user } = props
	...
	const handleClick = () => {
		...
		if (!!rollCallList) {
			...
			userList.forEach((data, i) => {
				...
				formData.push({ userID: data.userID, user: data.name, image: imageDataURL })
			})
			rollcall(formData)
				.then(data => {
					...
					{rollCallList?.map((element, ind) => {
						return (
							<div key={ind} style={{ display: 'flex', justifyContent: 'space-between' }}>
								<p>{element.name + ((element.name === user.name) ? ' (you)' : '')}</p>
								<p>{element.verified === 1 ? 'present' : 'absent'}</p>
							</div>
						)
					})}
				</Modal.Body>
